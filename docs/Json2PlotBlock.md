...
- **颜色优化**：
  - 使用预定义的高对比度颜色调色板
  - 不同系列（不同数据类型）使用不同颜色，确保系列间颜色差异明显
  - 颜色数量根据 measures 数量自动分配
...
- **颜色优化**：
  - 使用预定义的高对比度颜色调色板，包含多种色相、亮度和饱和度
  - 通过颜色排列算法，确保相邻扇形区域颜色差异明显
  - 算法基于色相环分布，相邻颜色在色相、亮度、饱和度上都有明显差异
  - 颜色调色板包含 20+ 种颜色，支持大数据量场景
  - 当数据项超过调色板数量时，自动循环使用颜色
