...
const LeftHeaderTool: React.FC<LeftHeaderToolProps> = ({ agentInfo, apiMethods, showAside = true }) => {
  ...
  const knowledgeNetworks = dataSource?.knowledge_network?.map(item => ({ id: item.knowledge_network_id, name: item?.name })) || [];
  const metricIds = dataSource?.metric?.map(item => item.metric_model_id) || [];

  useEffect(() => {
    console.log('agentInfo', agentInfo);
    console.log('knowledgeNetworks', knowledgeNetworks);
    console.log('metricIds', metricIds);
  }, [knowledgeNetworks, metricIds]); 
